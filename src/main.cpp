...
int main(void)
{
    ...
    Vector2 next2[4]
    {
     (next1[0] + next1[1]) * 0.5f,
     (next1[1] + next1[2]) * 0.5f,
     (next1[2] + next1[3]) * 0.5f,
     (next1[3] + next1[0]) * 0.5f
    };

    Vector2 next3[4]
    {
     (next2[0] + next2[1]) * 0.5f,
     (next2[1] + next2[2]) * 0.5f,
     (next2[2] + next2[3]) * 0.5f,
     (next2[3] + next2[0]) * 0.5f
    };

    Vector2 next4[4]
    {
     (next3[0] + next3[1]) * 0.5f,
     (next3[1] + next3[2]) * 0.5f,
     (next3[2] + next3[3]) * 0.5f,
     (next3[3] + next3[0]) * 0.5f
    };
    Vector2 next5[4]
    {
     (next4[0] + next4[1]) * 0.5f,
     (next4[1] + next4[2]) * 0.5f,
     (next4[2] + next4[3]) * 0.5f,
     (next4[3] + next4[0]) * 0.5f
    };

    Vector2 next6[4]
    {
     (next5[0] + next5[1]) * 0.5f,
     (next5[1] + next5[2]) * 0.5f,
     (next5[2] + next5[3]) * 0.5f,
     (next5[3] + next5[0]) * 0.5f
    };

    Vector2 next7[4]
    {
     (next6[0] + next6[1]) * 0.5f,
     (next6[1] + next6[2]) * 0.5f,
     (next6[2] + next6[3]) * 0.5f,
     (next6[3] + next6[0]) * 0.5f
    };

    Vector2 next8[4]
    {
     (next7[0] + next7[1]) * 0.5f,
     (next7[1] + next7[2]) * 0.5f,
     (next7[2] + next7[3]) * 0.5f,
     (next7[3] + next7[0]) * 0.5f
    };

    Vector2 next9[4]
    {
     (next8[0] + next8[1]) * 0.5f,
     (next8[1] + next8[2]) * 0.5f,
     (next8[2] + next8[3]) * 0.5f,
     (next8[3] + next8[0]) * 0.5f
    };

    Vector2 next10[4]
    {
     (next9[0] + next9[1]) * 0.5f,
     (next9[1] + next9[2]) * 0.5f,
     (next9[2] + next9[3]) * 0.5f,
     (next9[3] + next9[0]) * 0.5f
    };
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        case 3:
            ...
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next1);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next2);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next3);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next4);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next5);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next6);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next7);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next8);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next9);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(Vector2), next10);
            glDrawArrays(GL_LINE_LOOP, 0, 4);
            break;
